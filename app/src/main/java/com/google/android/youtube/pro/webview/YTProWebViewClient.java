...
public class YTProWebViewClient extends WebViewClient {
	...
	private final YTProWebView web;
		private final String nativeEnhancementScript;
	private final SharedPreferences prefs;
	
	public YTProWebViewClient(MainActivity activity, YTProWebView web) {
		this.activity = activity;
		this.web = web;
		this.prefs = activity.getSharedPreferences("YTPRO", android.content.Context.MODE_PRIVATE);
			this.nativeEnhancementScript = loadAssetScript("native_enhancements.js");
	}

	private int getNetworkTimeoutMs() {
		int sec = prefs.getInt("network_timeout_seconds", 20);
		sec = Math.max(5, Math.min(120, sec));
		return sec * 1000;
	}

		private String loadAssetScript(String assetName) {
			try (InputStream inputStream = activity.getAssets().open(assetName);
				...
				Log.e("YTPRO_WVC", "Failed to load native enhancements", e);
				return "";
			}
		}
    ...
    url.contains("SetSID")) {             
		...
		if (request.isForMainFrame() && (url.contains("m.youtube.com") || url.contains("www.youtube.com"))) {
			try {
				int timeoutMs = getNetworkTimeoutMs();
	...
	public void onPageFinished(WebView view, String url) {
		...
		if (!nativeEnhancementScript.isEmpty()) {
			web.evaluateJavascript(nativeEnhancementScript, null);
		}
