...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (prefs.getBoolean("enable_conscrypt", false)) {
    ...
    public void onPictureInPictureModeChanged(boolean isInPictureInPictureMode, Configuration newConfig) {
        web.evaluateJavascript(isInPictureInPictureMode
                ? "try{PIPlayer();}catch(e){};try{var v=document.querySelector('video.video-stream')||document.querySelector('video');if(v){v.style.visibility='visible';v.style.opacity='1';v.style.display='block';v.style.objectFit='contain';v.play().catch(()=>{});}}catch(e){}"
                : "try{removePIP();}catch(e){}", null);
