...
public class WebAppInterface {
	...
	public void getSNlM0e(String cookies) {
		new Thread(() -> {
			...
			String safeResponse = JSONObject.quote(response != null ? response : "");
			activity.runOnUiThread(() -> web.evaluateJavascript("callbackSNlM0e.resolve(" + safeResponse + ")", null));
		}).start();
	}
	...
	public void GeminiClient(String url, String headers, String body) {
		new Thread(() -> {
			...
			String safeResponse = JSONObject.quote(response != null ? response.toString() : "{}");
			activity.runOnUiThread(() -> web.evaluateJavascript("callbackGeminiClient.resolve(JSON.parse(" + safeResponse + "))", null));
		}).start();
	}
	...
	public String getAllCookies(String url) {
		...
		if (cookies == null || cookies.isEmpty()) {
			...
			for (String fallback : fallbackUrls) {
				cookies = cookieManager.getCookie(fallback);
				if (cookies != null && !cookies.isEmpty()) {
					break;
				}
			}
