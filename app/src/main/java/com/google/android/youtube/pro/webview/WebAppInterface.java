...
import com.google.android.youtube.pro.utils.NetworkCompat;
...
public class WebAppInterface {
	...
	@JavascriptInterface
	public void setEnableConscrypt(boolean enabled) {
		activity.getSharedPreferences("YTPRO", Context.MODE_PRIVATE)
				.edit()
				.putBoolean("enable_conscrypt", enabled)
				.apply();
		if (enabled) {
			NetworkCompat.installConscryptIfAvailable();
		}
	}

	@JavascriptInterface
	public boolean getEnableConscrypt() {
		return activity.getSharedPreferences("YTPRO", Context.MODE_PRIVATE)
				.getBoolean("enable_conscrypt", false);
	}

	@JavascriptInterface
	public void setNetworkTimeoutSeconds(int seconds) {
		int safe = Math.max(5, Math.min(120, seconds));
		activity.getSharedPreferences("YTPRO", Context.MODE_PRIVATE)
				.edit()
				.putInt("network_timeout_seconds", safe)
				.apply();
	}
	...
	public int getNetworkTimeoutSeconds() {
		return activity.getSharedPreferences("YTPRO", Context.MODE_PRIVATE)
				.getInt("network_timeout_seconds", 20);
	}

	@JavascriptInterface
	public void setPreferredDnsProvider(String provider) {
		String safe = "system";
		if ("google".equalsIgnoreCase(provider) || "cloudflare".equalsIgnoreCase(provider) || "system".equalsIgnoreCase(provider)) {
			safe = provider.toLowerCase();
		}
		activity.getSharedPreferences("YTPRO", Context.MODE_PRIVATE)
				.edit()
				.putString("preferred_dns_provider", safe)
				.apply();
	}

	@JavascriptInterface
	public String getPreferredDnsProvider() {
		return activity.getSharedPreferences("YTPRO", Context.MODE_PRIVATE)
				.getString("preferred_dns_provider", "system");
	}
